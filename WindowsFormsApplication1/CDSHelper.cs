...
namespace WindowsFormsApplication1
{
    ...
    public class CDSHelper
    {
        ...
        {
            string endPointURI = _APIURL + "admin-api/IoTDevice/" + Form1.GatewayID + "/Message";
            ...
            {
                if (message.MessageCatalogName == Form1.MessageName)
                {
                    applied = true;
                    break;
                }
            ...
            {
                //apply message
                endPointURI = _APIURL + "admin-api/IoTDevice/" + Form1.GatewayID + "/Message";

                var applyMessage = new
                {
                    CompanyId = Form1.CompanyId,
                    RequesterEmail = Form1.username,
                    MessageCatalogIdList = new int[] { Form1.MessageCatalogId }
                };
        ...
        public async Task<bool> GetMessaegCatalogId()
        {
            ...
            bool success = false;
                ...
                if (found == false&&((Form1.MessageName == "VCM") || (Form1.MessageName == "Dynamon") || (Form1.MessageName == "GMLite")))
                {
                    success = await CreateMessageCatalog(Form1.MessageName);
                    if (success == true)
                    {
                        found = await getMessageCatalogId();
                        if (found == false)
        ...
        {
            string endPointURI = _APIURL + "admin-api/WidgetCatalog/" + Form1.MessageCatalogId + "/" + Form1.MessageName;
        ...
        {
            string endPointURI = _APIURL + "admin-api/MessageCatalog/company/" + Form1.CompanyId.ToString();
            ...
            foreach (dynamic obj in IoTDeviceJSONObj)
            {
                ...
                if (obj.Name == Form1.MessageName)
                {
                    Form1.MessageCatalogId = obj.Id;
                    found = true;
                    break;
                }
            }
        ...
        {
            string endPointURI = _APIURL + "admin-api/Equipment/company/" + Form1.CompanyId.ToString();
            string IoTDeviceString = await callAPIService("GET", endPointURI, null);
            dynamic IoTDeviceJSONObj = JsonConvert.DeserializeObject(IoTDeviceString);
            Form1.eqId.Clear();
            foreach (dynamic obj in IoTDeviceJSONObj)
            {
                Form1.eqId.Add(Convert.ToString(obj.Name));
            }
        ...
        private async Task<bool> getAPIToken()
        {
            ...
            try
            {
                ...
                Form1.CompanyId = access_result.CompanyId;
                return true;
            }
            catch (Exception e)
            {
